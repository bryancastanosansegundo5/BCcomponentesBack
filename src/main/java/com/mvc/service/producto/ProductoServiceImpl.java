...
public class ProductoServiceImpl implements ProductoService {
    ...
    public Map<String, List<ProductoVO>> getRecomendaciones(List<Long> productoIds, Long usuarioId) {
        ...
        if (usuarioId == null || productoIds == null || productoIds.isEmpty()) {
            // 1. Cargar productos más vendidos y activos
        ...
        // 🕑 Productos comprados recientemente por el usuario (últimos 2 años)
        ...
        // 🔍 Separar productos vistos por el usuario: comprados y no comprados
        ...
        // 📂 Categorías de productos ya comprados (para excluir en complementarios)
        ...
        // ✅ COMPLEMENTARIOS: productos compatibles con lo que compró, de otras
        // categorías
        ...
        // 🔄 Agrupar por categoría para mezclar
        Map<Long, List<ProductoVO>> agrupados = complementariosBrutos.stream()
                .collect(Collectors.groupingBy(p -> p.getCategoria().getId()));

        // 🔁 Seleccionar hasta 10 complementarios alternando entre categorías
        ...
        // 🔁 Fallback si no hay suficientes resultados personalizados
        List<ProductoVO> masVendidos = productoRepository.findMasVendidosOrdenados().stream()
                .filter(p -> !p.isBaja())
                .toList();

        // 👉 Rellenar similares si están vacíos o incompletos
        ...
        // 👉 Rellenar complementarios si están vacíos
    ...
    public void importarDesdeExcel(MultipartFile archivoExcel) {
        try (InputStream inputStream = archivoExcel.getInputStream();
            ...
            for (int indiceFila = 1; indiceFila <= hojaProductos.getLastRowNum(); indiceFila++) {
                ...
                productoDTO.setVendidos(vendidos != null ? vendidos.intValue() : 0); // o lanza excepción si es
                                                                                     // obligatorio
    ...
    public byte[] generarPlantillaExcel() {
        try (Workbook libroExcel = new XSSFWorkbook()) {
            ...
            String[] valoresEjemplo = {
                    "Intel Core i5-11400F", "Procesador 6 núcleos, 12 hilos", "Socket LGA1200", "179.99", "10", "21",
                    "0",
                    "LGA1200", "", "", "", "150", "", "0",
